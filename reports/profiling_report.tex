...
\begin{itemize}[noitemsep, leftmargin=*]
    ...
    \item \textbf{Inputs}: 901 LFW test pairs from \fpath{outputs/pairs/test.csv},
...
launch produces \fpath{outputs/profiling/per_stage_latency.json},
\fpath{outputs/profiling/batch_size_sensitivity.csv},
\fpath{outputs/profiling/profiling_environment.json}, and a
human-readable \fpath{outputs/profiling/profiling_summary.md}.
...
\bottomrule
\end{tabular}
\caption{Per-pair latency, $n=25$ distinct pairs.
Source: \texttt{outputs/profiling/per\_stage\_latency.json}.}
\end{table}
...
\toprule
\textbf{Batch} & \textbf{Pre} & \textbf{Embed} & \textbf{Score} & \textbf{E2E} & \textbf{E2E/pair} & \textbf{Pairs/s} \\
                & \textbf{(ms)} & \textbf{(ms)} & \textbf{(ms)} & \textbf{(ms)} & \textbf{(ms)} & \\
\midrule
...
Columns: preprocessing total, embedding total, scoring total, end-to-end total,
end-to-end per pair, throughput. Source: \texttt{outputs/profiling/batch\_size\_sensitivity.csv}.}
\end{table}
...
\fpath{outputs/profiling/}. Re-running the same command on the same host gives
results within the noise band shown by the min/max columns. See
\fpath{reports/reproducibility_checklist.md} for the full release-level path.
