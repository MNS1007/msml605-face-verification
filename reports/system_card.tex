...
% Path macro that wraps at "/" and "_" inside running text.
...
\begin{itemize}[noitemsep, leftmargin=*]
    ...
    (\fpath{outputs/pairs_improved/}); the released system reports metrics on the
...
(see \fpath{outputs/threshold/selected_threshold.json}, also written into
\fpath{configs/m3.yaml::inference.threshold}).
...
(see \fpath{outputs/eval/test_confusion_matrix.json}).

\textbf{Confidence.} Calibrated to $[0.5, 1.0]$ as
$0.5 + 0.5 \cdot |score - \theta| / R$, where $R$ is the score range on the
relevant side of $\theta$. So $0.5$ means ``borderline'' and $1.0$ means
``score is at the extreme of its range.'' This is a deterministic mapping with
no learned parameters; it does not produce calibrated probabilities of identity.
...
\begin{itemize}[noitemsep, leftmargin=*]
    ...
    $\approx 22.6$ req/s with stable P95 (\fpath{outputs/load_test/runtime_summary.json}).
\end{itemize}

\section{Reproducibility Pointer}
\begin{itemize}[noitemsep, leftmargin=*]
    \item \textbf{Repository}: this directory; final tag \texttt{v1.0-final}.
    \item \textbf{Reproducibility checklist}: \fpath{reports/reproducibility_checklist.md}.
    \item \textbf{Profiling report}: \fpath{reports/profiling_report.pdf} (sources in \fpath{reports/profiling_report.tex}; raw artifacts in \fpath{outputs/profiling/}).
    \item \textbf{README}: top-level \texttt{README.md} -- copy-pastable setup, CLI, Docker, and profiling commands.
    \item \textbf{Final config}: \fpath{configs/m3.yaml} (seed, threshold $0.351759$, score range, preprocessing).
    \item \textbf{Selected threshold artifact}: \fpath{outputs/threshold/selected_threshold.json}.
    \item \textbf{Test metrics artifact}: \fpath{outputs/eval/test_metrics.json}.
\end{itemize}
